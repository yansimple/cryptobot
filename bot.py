...
def callback_inline(call):
    try:
        ...
        if call.data == "answ2":
            ...
            keyboard.add(callback_button3)
            keyboard.add(callback_button4)
            bot.send_message(call.message.chat.id, "Jaki poziom dochodów jest Twoim ostatecznym celem?",
                             reply_markup=keyboard)

        if call.data == "answ3":
            keyboard = types.InlineKeyboardMarkup()
            callback_button1 = types.InlineKeyboardButton(text="Jestem osobą spontaniczną", callback_data="answ4")
            callback_button2 = types.InlineKeyboardButton(text="Gromadząc i analizując jak najwięcej informacji",
                                                          callback_data="answ4")
            callback_button3 = types.InlineKeyboardButton(text="Konsultując się z ekspertami / profesjonalistami",
                                                          callback_data="answ4")
            callback_button4 = types.InlineKeyboardButton(text="Konsultując się z rodziną lub przyjaciółmi",
                                                          callback_data="answ4")
            keyboard.add(callback_button1)
            keyboard.add(callback_button2)
            keyboard.add(callback_button3)
            keyboard.add(callback_button4)
            bot.send_message(call.message.chat.id, "Jak podchodzisz do podejmowania strategicznych decyzji?",
                             reply_markup=keyboard)
        ...
        if call.data == "answ4":
            ...
            bot.send_message(call.message.chat.id,
                             "Sądząc z własnego doświadczenia, jak skuteczne były Twoje inwestycje w tych dziedzinach?",
                             reply_markup=keyboard)
        ...
        if call.data == "answ5":
            ...
            callback_button3 = types.InlineKeyboardButton(text="Kupiłbym udziały w odnoszącej sukcesy firmie",
                                                          callback_data="answ6")
            callback_button4 = types.InlineKeyboardButton(text="Spędziłbym to wszystko z przyjaciółmi",
                                                          callback_data="answ6")
        ...
        if call.data == "answ6":
            ...
            keyboard.add(callback_button3)
            keyboard.add(callback_button4)
            bot.send_message(call.message.chat.id, "Ile masz kapitału na swoją początkową inwestycję?",
                             reply_markup=keyboard)

        if call.data == "answ7":
            keyboard = types.InlineKeyboardMarkup()
            url_button = types.InlineKeyboardButton(text="Twój wiek",
                                                    url="https://dijaceqa.momonito.com/tracker?offer_id=3836&aff_id=9756")

            keyboard.add(url_button)

            bot.send_message(call.message.chat.id,
                             "Proszę wprowadzić swój wiek. Aby wziąć udział, musisz mieć ukończone 18 lat",
                             reply_markup=keyboard)
    except:
        print('error')
